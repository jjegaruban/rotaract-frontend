...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-white shadow-md">
      <div className="max-w-7xl mx-auto flex justify-between items-center px-6 md:px-10 h-32">
        {/* Logo */}
        <Link to="/" className="flex items-center mr-10">
          <img src={logo} alt="Rotaract Logo" className="h-28 w-auto" />
        </Link>
      ...
      {isOpen && (
        <div
          ref={menuRef}
          className="md:hidden bg-white shadow-lg w-full absolute top-32 left-0"
        >
          {navItems.map((item) => (
            <Link
              key={item.name}
              to={item.path}
              className={`block px-6 py-4 border-b border-gray-100 text-base font-normal transition-colors ${
                location.pathname === item.path
                  ? "text-[#d22163] bg-[#fef2f7]"
                  : "text-gray-800 hover:text-[#d22163] hover:bg-[#fef2f7]"
              }`}
